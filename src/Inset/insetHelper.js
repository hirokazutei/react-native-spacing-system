...
export function convertInsetPaddingKeyToValue<T>({
  ...
  keyedPaddings: PaddingPossibilities<T>,
  spacing: { [key: T]: number },
  debugOptions?: InsetDebugOptions
}) {
  const all = keyedPaddings.all
    ? {
        default: { padding: spacing[keyedPaddings.all] },
        debug: { borderWidth: spacing[keyedPaddings.all] }
      }
    : {};
  const horizontal = keyedPaddings.horizontal
    ? {
        default: { paddingHorizontal: spacing[keyedPaddings.horizontal] },
        debug: {
  ...
  const vertical = keyedPaddings.vertical
    ? {
        default: { paddingVertical: spacing[keyedPaddings.vertical] },
        debug: {
  ...
  const top = keyedPaddings.top
    ? {
        default: { paddingTop: spacing[keyedPaddings.top] },
        debug: {
  ...
  const right = keyedPaddings.right
    ? {
        default: { paddingRight: spacing[keyedPaddings.right] },
        debug: {
  ...
  const bottom = keyedPaddings.bottom
    ? {
        default: { paddingBottom: spacing[keyedPaddings.bottom] },
        debug: {
  ...
  const left = keyedPaddings.left
    ? {
        default: { paddingLeft: spacing[keyedPaddings.left] },
        debug: {
...
export const obtainInsetPaddingStyle = ({
  ...
  const all = paddings.all
    ? {
        default: { padding: paddings.all },
        debug: { borderWidth: paddings.all }
      }
    : {};
  const horizontal = paddings.horizontal
    ? {
        default: { paddingHorizontal: paddings.horizontal },
        debug: {
  ...
  const vertical = paddings.vertical
    ? {
        default: { paddingVertical: paddings.vertical },
        debug: {
  ...
  const top = paddings.top
    ? {
        default: { paddingTop: paddings.top },
        debug: {
  ...
  const right = paddings.right
    ? {
        default: { paddingRight: paddings.right },
        debug: {
  ...
  const bottom = paddings.bottom
    ? {
        default: { paddingBottom: paddings.bottom },
        debug: {
  ...
  const left = paddings.left
    ? {
        default: { paddingLeft: paddings.left },
        debug: {
